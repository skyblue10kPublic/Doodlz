...
public class DoodleView extends View {
   ...
   private int lastBackgroundColor = Color.WHITE;
   private float selectedLineWidthMax = 5;
   private double  normalizedStrokeWidth = 0;
   private boolean determinedWidth = false;
   ...
   public DoodleView(Context context, AttributeSet attrs) {
      ...
      paintLine.setStrokeWidth(selectedLineWidthMax); // set the default line width
   ...
   public void clearUseCustomColor(int color)
   {
      pathMap.clear(); // remove all paths
      previousPointMap.clear(); // remove all previous points
   ...
   public int getLineWidth() {
      ...
      //return Math.min(1,Math.round(normalizedStrokeWidth));
   ...
   public boolean onTouchEvent(MotionEvent event) {
      ...
      //float pressureMagnitude = event.getPressure(actionIndex);

      paintLine.setStrokeWidth((float)normalizedStrokeWidth);
         ...
         if (pathMap.containsKey(pointerID)) {
            ...
            if (deltaX >= TOUCH_TOLERANCE || deltaY >= TOUCH_TOLERANCE) {
               ...
               if (!determinedWidth) {
                  ...
                  normalizedStrokeWidth = Math.min(distance, 12) / 12;
                  normalizedStrokeWidth = normalizedStrokeWidth * (double) selectedLineWidthMax;
                  //stackAverage.push(normalizedStrokeWidth);
                  paintLine.setStrokeWidth(Math.min(1,Math.round(normalizedStrokeWidth)));
                  determinedWidth = true;
               }
   ...
   private void touchEnded(int lineID) {
      ...
      while (!stackAverage.isEmpty())
      {
         average += stackAverage.pop();
         items++;
      }
      ...
      paintLine.setStrokeWidth(Math.round(average));
      */
      bitmapCanvas.drawPath(path, paintLine); // draw to bitmapCanvas
      //paintLine.setStrokeWidth(selectedLineWidthMax);
      determinedWidth = false;
